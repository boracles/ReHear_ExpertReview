...
export function ExpertReviewForm({ participantId, reviewToken, verifiedEmail }: { participantId: string; reviewToken: string; verifiedEmail: string }) {
  ...
  const [activeVideoStage, setActiveVideoStage] = useState<Record<VideoKey, string>>({ A: videoStages[0].key, B: videoStages[0].key });
  ...
  const completion = useMemo(() => {
    ...
    return Math.round(((model + videos) / (modelCriteria.length + (videoStages.length * 2))) * 100);
  }, [data.overall]);
  ...
  return <section className="review-workspace" id="review-workspace" aria-labelledby="review-title">
    ...
    <section className="form-section reference-section" aria-labelledby="materials-title"><div className="legend-like"><span>04</span><div><b id="materials-title">제공 자료 및 검토 순서</b><small>독립 검토 전 제공되는 자료와 이후 진행 순서입니다.</small></div></div><div className="materials-layout"><article className="reference-card materials-card"><p className="reference-label">PROVIDED MATERIALS</p><h3>검토 전 제공 자료</h3><ul className="materials-list"><li>프레임워크 V1 개요</li><li>E·V·C 청중 상태 모델 설명 및 도식</li><li>발표 수행정보·평가 차원 코드북</li><li>발표 단계 구분과 시간 정보</li><li>단계별 평가 관점의 정의와 이론적 근거</li><li>VR 발표 영상 샘플 A·B</li><li>구현 제약 설명</li></ul><div className="review-duration-summary"><span>총 소요시간</span><b>약 60–80분</b></div></article><div className="review-steps"><article><div className="review-step-icon review-step-independent" aria-hidden="true"><i className="person-glyph" /><i className="document-glyph">✓</i></div><div><small>INDEPENDENT REVIEW</small><h3>1단계 독립 검토 <em>약 30–40분</em></h3><p>다른 전문가의 점수나 연구자의 선호를 알지 못한 상태에서 평가표를 작성하고, VR 발표 영상 샘플 A·B를 보며 발표 흐름에 따른 AI 청중의 반응을 검토합니다.</p></div></article><article><div className="review-step-icon review-step-interview" aria-hidden="true"><i className="person-glyph person-one" /><i className="person-glyph person-two" /><i className="speech-glyph">···</i></div><div><small>FOLLOW-UP INTERVIEW</small><h3>2단계 후속 면담 <em>약 30–40분</em></h3><p>독립 검토 응답을 바탕으로 점수의 근거, 단계별 평가 관점, 발표 수행정보–청중 상태–백채널 표현의 연결, 의미 해석, 청중 반응의 변화, 누락·중복된 구성요소 및 구현상 위험을 구체적인 사례 중심으로 확인합니다.</p></div></article></div></div></section>

    <section className="form-section reference-section model-overview" aria-labelledby="model-title">
      <div className="legend-like"><span>05</span><div><b id="model-title">E·V·C 청중 상태 모델 및 백채널 표현 구조 개요</b><small>평가 대상 모델의 구성과 정보 흐름을 확인해주세요.</small></div></div>
      <div className="model-target"><span>검토 대상</span><p>발표 수행정보를 바탕으로 AI 청중의 E·V·C 상태를 산출·갱신하고, 이를 비언어적 백채널로 표현하는 청중 상태 모델 및 표현 구조</p></div>
      <div className="evc-grid">
        <article data-state="E"><span>E</span><small>ENGAGEMENT</small><h3>Engagement</h3><p>발표 상황과 발표자에게 주의를 기울이고 관여하는 정도</p></article>
        <article data-state="V"><span>V</span><small>EVALUATIVE VALENCE</small><h3>Evaluative Valence</h3><p>발표 내용 또는 수행에 대한 긍정적·부정적 평가 방향</p></article>
        <article data-state="C"><span>C</span><small>COGNITIVE CLARITY</small><h3>Cognitive Clarity</h3><p>발표 내용과 구조를 이해하는 정도</p></article>
      </div>
      <figure className="evc-state-space" aria-labelledby="evc-space-title">
        <figcaption><small>THREE-DIMENSIONAL STATE SPACE</small><h3 id="evc-space-title">E·V·C 3축 상태 공간</h3><p>각 AI 청중 에이전트의 상태와 시간에 따른 갱신을 하나의 좌표 공간에서 확인합니다.</p></figcaption>
        <div className="state-space-canvas" role="img" aria-label="Engagement, Evaluative Valence, Cognitive Clarity 세 축 위에서 세 AI 청중 에이전트의 상태가 t−1에서 t로 이동하는 개념도">
          <span className="state-axis axis-engagement"><i>낮음</i><b>Engagement</b><em>높음</em></span>
          <span className="state-axis axis-valence"><i>부정</i><b>Evaluative Valence</b><em>긍정</em></span>
          <span className="state-axis axis-clarity"><i>낮음</i><b>Cognitive Clarity</b><em>높음</em></span>
          <span className="state-origin" aria-hidden="true" />
          <span className="agent-path agent-one" aria-hidden="true"><i /><b /><em>Agent 1</em></span>
          <span className="agent-path agent-two" aria-hidden="true"><i /><b /><em>Agent 2</em></span>
          <span className="agent-path agent-three" aria-hidden="true"><i /><b /><em>Agent 3</em></span>
          <span className="time-key"><i />t−1 <b>→</b> <em />t</span>
        </div>
        <p className="state-space-note">각 AI 청중 에이전트의 상태는 E·V·C 세 값의 조합으로 표현되며, 발표 수행정보·이전 상태·개별 에이전트 특성에 따라 시간에 따라 갱신됩니다. 본 도식은 개념적 표현이며 세 차원의 통계적 독립성을 전제하지 않습니다.</p>
      </figure>
      <div className="presentation-stage-strip" aria-label="발표 단계 구조">{videoStages.map((stage, index) => <span key={stage.key}><b>{String(index + 1).padStart(2, "0")}</b>{stage.label}<small>{stage.status}</small></span>)}</div>
      <dl className="model-facts"><div><dt>입력</dt><dd>발표 단계별 내용 평가 결과와 음성·머리 방향 기반 시선 등 전달 수행 평가 결과</dd></div><div><dt>개별 에이전트 특성</dt><dd>발표 주제에 대한 관심도와 배경지식</dd></div><div><dt>상태 산출 및 갱신</dt><dd>현재의 발표 수행정보, 이전 청중 상태 및 개별 에이전트 특성을 종합하여 각 AI 청중 에이전트의 E·V·C 상태를 산출하고 갱신</dd></div><div><dt>백채널 표현</dt><dd>E·V·C 상태를 표정·자세·시선·고개 움직임으로 표현</dd></div><div><dt>제공 예시</dt><dd>발표자의 수행에 실시간으로 반응하는 AI 청중이 포함된 VR 발표 영상 샘플 2개와 발표 단계 표시</dd></div></dl>
      <details className="theory-references"><summary>관련 이론 참고문헌 확인하기</summary><p>E·V·C는 본 연구에서 구성한 청중 상태 모델입니다. 아래 문헌은 상태 평가 과정과 동적인 반응 갱신에 관한 관련 이론적 배경을 확인하기 위한 자료입니다.</p><ul><li><a href="https://doi.org/10.1093/oso/9780195130072.003.0005" target="_blank" rel="noreferrer">Scherer (2001), Appraisal Considered as a Process of Multilevel Sequential Checking</a><span>Oxford University Press · DOI 원문 페이지</span></li><li><a href="https://doi.org/10.1080/02699930902928969" target="_blank" rel="noreferrer">Scherer (2009), The Dynamic Architecture of Emotion</a><span>Cognition and Emotion, 23(7), 1307–1351 · DOI 원문 페이지</span></li></ul></details>
    </section>
    ...
    <section className="form-section framework-evaluation" aria-labelledby="framework-evaluation-title">
      <div className="legend-like"><span>07</span><div><b id="framework-evaluation-title">VR 발표 영상 구간별 평가</b><small>영상 1의 구간별 문항을 완료한 뒤 영상 2를 같은 방식으로 평가해주세요.</small></div></div>
      ...
      <div className="framework-review-layout">
        <section className="framework-video-section" aria-labelledby="video-samples-title">
          <div className="framework-video-heading"><p className="reference-label">VIDEO REVIEW · SAMPLE 01–02</p><h3 id="video-samples-title">VR 발표 영상별 AI 청중 반응 검토</h3><p>영상에 표시되는 발표 단계와 같은 구간을 아래에서 선택하면 해당 구간의 문항이 바로 나타납니다. 영상 1의 모든 구간을 평가한 뒤 영상 2를 같은 방식으로 진행해주세요.</p></div>
          ...
          <div className="framework-video-list">{(["A", "B"] as VideoKey[]).map((video, videoIndex) => {
            ...
            const activeStageIndex = Math.max(0, videoStages.findIndex((stage) => stage.key === activeVideoStage[video]));
            const stage = videoStages[activeStageIndex];
            const answer = data.overall.videos[video][stage.key];
            return <article className={`framework-video-card ${isComplete ? "is-complete" : ""}`} id={`video-review-${video.toLowerCase()}`} key={video}>
              <header><span>{String(videoIndex + 1).padStart(2, "0")}</span><div><small>VIDEO SAMPLE {video}</small><h4>영상 {videoIndex + 1} 시청 및 구간별 평가</h4></div><em>{isComplete ? "8개 구간 평가 완료" : `${completedStages}/8개 구간 입력`}</em></header>
              ...
              <section className="video-inline-evaluation" aria-label={`영상 ${videoIndex + 1} 단계별 평가`}>
                <div className="video-evaluation-heading"><div><small>SELECT A SEGMENT &amp; RATE</small><h5>영상 {videoIndex + 1} 구간별 평가</h5></div><span>{completedStages}/{videoStages.length} 구간</span></div>
                <p className="stage-selector-help">영상에 표시되는 현재 발표 단계와 같은 구간을 선택해주세요. 완료한 구간에는 체크가 표시됩니다.</p>
                <div className="video-stage-selector" role="tablist" aria-label={`영상 ${videoIndex + 1} 평가 구간`}>
                  {videoStages.map((item, stageIndex) => { const completed = Boolean(data.overall.videos[video][item.key].rating); const active = item.key === stage.key; return <button type="button" role="tab" aria-selected={active} className={`${active ? "active" : ""} ${completed ? "done" : ""}`} key={item.key} onClick={() => setActiveVideoStage((current) => ({ ...current, [video]: item.key }))}><span>{completed ? "✓" : String(stageIndex + 1).padStart(2, "0")}</span><b>{item.label}</b><small>{item.status}</small></button>; })}
                </div>
                <div className="video-stage-list" aria-live="polite"><article className="stage-evaluation active-stage-evaluation" key={`${video}-${stage.key}`}>
                  <div className="stage-head"><span>{String(activeStageIndex + 1).padStart(2, "0")}</span><div><small>현재 평가 구간 · {stage.status}</small><h4>{stage.label}</h4></div></div>
                  ...
                  <label className="field"><span>판단 근거·보완 의견</span><textarea rows={3} value={answer.comment} onChange={(event) => updateVideoStage(video, stage.key, { comment: event.target.value })} /></label>
                  <div className="stage-navigation">
                    <button type="button" disabled={activeStageIndex === 0} onClick={() => setActiveVideoStage((current) => ({ ...current, [video]: videoStages[activeStageIndex - 1].key }))}>← 이전 구간</button>
                    {activeStageIndex < videoStages.length - 1 ? <button type="button" className="primary" onClick={() => setActiveVideoStage((current) => ({ ...current, [video]: videoStages[activeStageIndex + 1].key }))}>다음 구간 <span>→</span></button> : <button type="button" className="primary" onClick={() => document.getElementById(video === "A" ? "video-review-b" : "review-submit")?.scrollIntoView({ behavior: "smooth", block: "start" })}>{video === "A" ? "영상 2로 이동" : "평가 작성 마치기"}<span>→</span></button>}
                  </div>
                </article></div>
              </section>
            </article>;
